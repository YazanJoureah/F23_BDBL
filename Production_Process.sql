...
AS
    total_cost NUMBER := 0; 
    Production_ID INT := 0; -- Variable to store the next available Production ID
    ...
    -- Declare a cursor to fetch the required data for each material to heck if raw materials are sufficient for the production 
...
CLOSE material_cursor_1;
    ...
    INSERT INTO Production (Production_ID,Machine_ID, Product_ID, Production_Date,Quantity_Produced) 
    VALUES (v_Production_ID,PR_Machine,PR_Product,SYSDATE, PR_Quantity);
